...
class image:
    ...
    def load(self):
        ...
        iface.addRasterLayer(self.file,name)

        #layer = QgsRasterLayer(self.georeferenced,name)
       # layer = QgsRasterLayer(self.file,name)

      #  group = getGroup(self.groups)#QgsLayerTreeGroup
      #  group.addLayer(layer)
      #  group.setExpanded(False)    
        
        #addLayer
        
       # QgsProject.instance().addMapLayer(layer,True)#don't immediatly add to legend
...
def gcpsRectangle(lineLayer,startField,endField,startM,endM,offset=0.0):
...
def gcpsPolygon(lineLayer,startField,endField,startM,endM,offset=0.0):
...
def remakeImages(images,progress,layer=None,startField=None,endField=None,useRectangle=True):
    if layer and startField and endField:
        ...
        for i,im in enumerate(images):
            ...
            if useRectangle:
                ...
                im.gcps = gcpsRectangle(lineLayer = layer,
                ...
                im.gcps = gcpsPolygon(lineLayer = layer,
                     startField = startField,
                     endField = endField,
                     startM = im.startM,
                     endM = im.endM,
